...
void generateWheelSides()
{
	...
	for ( double angle = 0; angle <= 360; angle += angleincrement)
	{
		...
		wheelSide1Verts[point++] = vec4(0.0f,		side*(-1.0f), 0.0f, 1.0); //point 1
		
		wheelside1Colors[point] = vec4(cos(angle*M_PI/180), -1.0f, -sin(angle*M_PI/180), 1.0); //point 2
		wheelSide1Verts[point++] = vec4(cos(angle*M_PI/180), side*(-1.0f), -sin(angle*M_PI/180), 1.0); //point 2
		
		wheelside1Colors[point] = vec4(cos((angle+angleincrement)*M_PI/180), -1.0f, -sin((angle+angleincrement)*M_PI/180), 1.0); //point 3
		wheelSide1Verts[point++] = vec4(cos((angle+angleincrement)*M_PI/180), side*(-1.0f), -sin((angle+angleincrement)*M_PI/180), 1.0); //point 3
	...
	for ( double angle = 0; angle <= 360; angle += angleincrement)
	{
		wheelSide2Verts[point++] = vec4(0.0f,		side*(-1.0f), 0.0f, 1.0); 
		

		wheelSide2Verts[point++] = vec4(cos(angle*M_PI/180), side*(-1.0f), -sin(angle*M_PI/180), 1.0); 
		wheelside2Colors[point] = vec4(0.5f,0.5f,0.5f, 1.0); 

		wheelSide2Verts[point++] = vec4(cos((angle+angleincrement)*M_PI/180), side*(-1.0f), -sin((angle+angleincrement)*M_PI/180), 1.0); //point 3
		wheelside2Colors[point] = vec4(0.5f ,0.5f,0.5f, 1.0); 
	...
	for ( int i = 0; i < 410 ; i += 6)
	{
		...
		wheelCylinderVers[i] =   wheelSide1Verts[(p+1)%3==0?(p+1+1+1):(p+1)];
		wheelCylinderVers[i+1] = wheelSide1Verts[(p+2)%3==0?(p+2+1+1):(p+2)];
		wheelCylinderVers[i+2] = wheelSide2Verts[(p+1)%3==0?(p+1+1+1):(p+1)];


		wheelCylinderVers[i+3] = wheelSide1Verts[(p+2)%3==0?(p+2+1+1):(p+2)];
		wheelCylinderVers[i+4] = wheelSide2Verts[(p+2)%3==0?(p+2+1+1):(p+2)];
		wheelCylinderVers[i+5] = wheelSide2Verts[(p+1)%3==0?(p+1+1+1):(p+1)];
	...
	glBufferData( GL_ARRAY_BUFFER, sizeof(wheelSide2Verts), wheelSide2Verts, GL_STATIC_DRAW);
	vPosition = glGetAttribLocation(program, "vPosition");
	glEnableVertexAttribArray(vPosition);
	glVertexAttribPointer(vPosition, 4, GL_FLOAT, GL_FALSE, 0, 0);
	...
	glBufferData( GL_ARRAY_BUFFER, sizeof(wheelCylinderVers), wheelCylinderVers, GL_STATIC_DRAW);
	vPosition = glGetAttribLocation(program, "vPosition");
	glEnableVertexAttribArray(vPosition);
	glVertexAttribPointer(vPosition, 4, GL_FLOAT, GL_FALSE, 0, 0);
